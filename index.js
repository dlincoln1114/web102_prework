...
const totalBackers = GAMES_JSON.reduce((acc,curr) => acc + curr.backers, 0);
...
const totalRaised = GAMES_JSON.reduce((acc,curr) => acc + curr.pledged,0)
...
const unfunded = GAMES_JSON.filter((games) => games.pledged < games.goal);
console.log(unfunded);

// create a string that explains the number of unfunded games using the ternary operator
const displayUnfundedGames = `The total number of games that's underfunded: ${unfunded.length > 0? unfunded.length: 'none'}`;

// create a new DOM element containing the template string and append it to the description container
const newDescription = document.createElement('p');
newDescription.innerHTML = displayUnfundedGames
